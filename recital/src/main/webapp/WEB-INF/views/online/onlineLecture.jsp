	              	...
	              	<div class="modal-content">
	                	<div class="modal-header">
	               		...
	               		<div class="modal-body">
		                  	<div class="row mb-3">
	                      		<input type="hidden" class="form-control" id="lecNo" value="${lecVO.lecNo }">
	                      		<input type="hidden" class="form-control" id="lecName" value="${lecVO.lecName }">
		                    	<div class="col-sm-12">
...
$(function(){
		...
		$.ajax({
			...
			success : function(res) {
				...
				$.each(res, function(i,v){
					if(v.roomTitle.indexOf("_") != -1 && v.roomTitle.split("_")[0] == "LEC"){
						var title = v.roomTitle.substring(4);
						console.log(title);
						if(title.indexOf("_") != -1 && (title.split("_")[0] + "_" + title.split("_")[1]) == $("#lecNo").val()){
	...
	function enterRoom(roomId, username, userid){
		$.ajax({
			type : 'post',
			url : "/online/enterRoom?roomId="+roomId+"&username="+username+"&userid="+userid,
			beforeSend : function(xhr){	// 데이터 전송 전, 헤더에 csrf 값 설정
				xhr.setRequestHeader(header, token);
			},
			success : function(res) {
	...
	registerBtn.on('click',function(){
		...
		var roomTitle = "LEC_" + $("#lecNo").val() + "_" + $("#roomTitle").val();
