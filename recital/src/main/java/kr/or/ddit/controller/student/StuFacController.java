...
//		List<FacilityVO> facilityList =  facilityService.selectFacility();
	...
	public ResponseEntity<String> facStuResDelete(FacilityReserveVO resVO){
		ResponseEntity<String> entity = null;		
		
		log.info("resVO 잘 찍힘? : " + resVO.getUserNo());
		log.info("resVO 잘 찍힘? : " + resVO.getFacResNo());
		
		int boolUser = service.facilityResBoolean(resVO);
		if(boolUser < 1) {
			entity = new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
		} else {
			int cnt = service.facStuResDelete(resVO.getFacResNo());
			if(cnt > 0) {
				entity = new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
			}else {
				entity = new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
